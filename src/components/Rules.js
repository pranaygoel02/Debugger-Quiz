...
function Rules() {
  ...
  return (
    ...
    <div className='w-ful h-full flex flex-col gap-4 items-center justify-start relative bg-gradient-to-t from-white to-blue-600 overflow-x-hidden'>
      {/* <div className='w-full h-56 bg-blue-800'></div> */}
      <div className='absolute top-[16%] flex flex-col gap-8 bg-white rounded border p-12 shadow-lg w-[80%]'>
        <div className='flex flex-col items-start text-start gap-8'>
          ...
          <ul className='text-[22px]'>
            {rules.map((rule, index) => <li key={index} className="mb-[5px]">{rule}</li>)}
          </ul>
        </div>
        <div className='flex gap-1'>
          ...
          <label htmlFor='check-rules' className='text-[22px] font-[600]'>I have read all rules, and agree to it.</label>
        </div>
