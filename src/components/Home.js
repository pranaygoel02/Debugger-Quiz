    ...
    user && (
      <div className="flex flex-col md:flex-row w-full h-full relative">
        ...
        >
          ...
          <p className="text-xl">
            Question: <span className="text-3xl font-bold">{id + 1}</span>/
            {questions?.length}
          </p>
          <h2 className="text-[22px]">{questions[id].title}</h2>
          <div
            id="question"
            className="select-none bg-white shadow-md text-black w-full p-4 max-h-96 overflow-auto rounded-lg text-[18px]"
          ></div>
        </div>
        <div
          className="flex flex-col items-start w-full text-start justify-center gap-4 p-16"
          style={{ flexBasis: "50%" }}
        >
          ...
          <div className="flex flex-col items-stretch justify-start gap-4 w-full transition-all">
            {questions[id].options.map((option, index) => {
              return (
                <label
                  key={index}
                  className={`rounded border w-full flex gap-2 p-4 hover:shadow-md hover:border-green-400 transition-all cursor-pointer ${
                    option === questions[id]?.chosenAns &&
                ...
                >
                  <input
                    ...
                    type="radio"
                    id={option}
                    defaultChecked={option === questions[id]?.chosenAns}
                    value={option}
                    name="radio"
                  />
                  <p>{option}</p>
        ...
        <div className="absolute top-0 left-0 gap-2 flex p-4">
          <button
            className="p-2 border rounded text-white hover:bg-white/20"
        ...
        <div className="absolute bottom-0 right-0 gap-2 flex p-4">
          ...
          <button
            disabled={id === questions.length - 1}
            className="p-2 bg-blue-600 rounded text-white"
        ...
        <div className="absolute top-0 right-0 gap-2 flex p-4">
          {questions[id]?.chosenAns !== undefined && (
            <button
              className="p-2 bg-blue-600 rounded text-white"
        ...
        <div className="absolute top-0 left-0 gap-2 flex p-4"></div>
      </div>
    )
  );
}
